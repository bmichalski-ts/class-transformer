            ...
            for (const key of keys) {
                ...
                if (!this.options.enableCircularCheck || !this.isCircular(subValue)) {
                    ...
                    if (this.transformationType === enums_1.TransformationType.CLASS_TO_PLAIN) {
                        ...
                        const customTransformationResult = this.applyCustomTransformations(value[transformKey], targetType, transformKey, value, this.transformationType);
                        ...
                        if (subValue === undefined && this.options.exposeDefaultValues) {
                            ...
                            const customTransformationResult = this.applyCustomTransformations(subValue, targetType, transformKey, value, this.transformationType);
                            if (customTransformationResult.customTransformationWasApplied) {
                                finalValue = customTransformationResult.value;
                            }
                            else {
                    ...
                    const { value: finalValue } = this.applyCustomTransformations(subValue, targetType, key, value, this.transformationType);
                    if (finalValue !== undefined || this.options.exposeUnsetFields) {
                        if (newValue instanceof Map) {
                            newValue.set(newValueKey, finalValue);
                        }
